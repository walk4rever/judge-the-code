...
4. **Monorepo 检测**：如果顶层目录中存在 `packages/`、`apps/`、`services/` 且各自包含多个子目录，判断为 monorepo。
   此时**暂停分析**，向用户询问：
   ```
...
5. 阅读 README.md（如存在）提取架构描述，最多读前 150 行

6. 检查 `docs/` 或 `documentation/` 目录
