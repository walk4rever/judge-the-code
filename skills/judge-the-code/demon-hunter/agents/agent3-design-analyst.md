...
优先复用已有分析，避免重复工作：
```bash
# 检查是否有 philosophy-extractor 的输出
ls PHILOSOPHY.md 2>/dev/null && echo "FOUND" || echo "MISSING"
ls UNDERSTANDING.md 2>/dev/null && echo "FOUND" || echo "MISSING"
```
- 若有 `PHILOSOPHY.md`：读取"隐含设计原则"和"存疑决策"，作为判断基准
- 若有 `UNDERSTANDING.md`：读取架构图和核心模块列表
...
若不存在：跳过此步，注明"建议先运行 /philosophy-extractor 获取更深层分析"
