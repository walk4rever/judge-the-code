...
- **Effort**: M
- **Status**: 🚧 规划完成,待实施 (见 `docs/code-explore-tool-selection.md`)

### P5 - 远期（skill-review：Skill/Prompt 工程审查）

#### 9. `skill-review` 子 skill 设计与实现
- **What**: 新建专属子 skill，审查 Skill/Prompt 工程项目（如 `~/.agents/skills/` 下的 skill）的质量。
- **Why**: 当前 4 个子 skill 全部围绕传统源代码设计，对 Skill 项目几乎完全不适用——`syft` 扫 markdown 无意义，`bearer` 找不到 CVE，`design-lens` 分析不了自然语言指令的设计哲学。但 Skill 项目同样需要审查，只是质量维度完全不同。
- **核心审查维度**:
  - Prompt 清晰度：指令是否有歧义？低智商模型会不会理解出不同结果？
  - 执行流设计：Phase 分层是否合理？有没有死路或信息断层？
  - 输入验证：Phase 0 是否覆盖异常输入？
  - Agent 编排：并行 Agent 是否真正独立？有没有隐式串行依赖？
  - 容错与降级：Agent 失败或工具缺失时有没有 fallback？
  - Token 自身效率：Prompt 本身是否臃肿？
  - 安全边界：Prompt Injection 风险？文件系统访问权限是否过宽？
  - 模型兼容性：是否过度依赖某个模型的特性？
- **架构方向**: 独立于现有 4 个子 skill，作为第二条审查路径。共享 dashboard 基础设施。
- **实施方案（已锁定）**:
  - **架构模式**: 采用“规则扫描（确定性）+ LLM 语义审查（经验判断）”混合架构，避免纯 Prompt 误判。
  - **入口策略**:
    - 新增 `skills/skill-review/SKILL.md`。
    - 自动识别 Skill 项目：目标目录存在 `SKILL.md` 或 `references/agent-specs/*.md` 时优先走 `skill-review` 路径。
  - **执行流（5 Phase）**:
    - Phase 0: 输入验证（EMPTY/NOT_FOUND/IS_FILE/OK，与现有子 skill 一致）。
    - Phase 1: 结构发现（枚举 `SKILL.md`、`references/agent-specs/*.md`、`setup`、`bin/*`）。
    - Phase 2: 规则扫描（输入校验、fallback、危险命令、防注入边界、并行依赖冲突、超长 Prompt），输出 JSON 证据。
    - Phase 3: LLM 语义审查（清晰度、执行流、模型兼容性、编排合理性）。
    - Phase 4: 综合输出（生成 `.judge-the-code/skill-review.md` 并刷新 dashboard）。
  - **模块拆分**:
    - `skills/skill-review/references/agent-specs/agent1-clarity.md`（Prompt 清晰度）
    - `skills/skill-review/references/agent-specs/agent2-flow.md`（Phase/流程设计）
    - `skills/skill-review/references/agent-specs/agent3-safety.md`（注入与权限边界）
    - `skills/skill-review/references/agent-specs/agent4-orchestration.md`（并行/依赖/容错）
  - **报告规范**:
    - 文件：`.judge-the-code/skill-review.md`
    - 内容：总分（0-100）+ 等级、8 维评分、按严重度分级的 Findings（含路径+行号+修复建议）、Top 5 Fixes（按 ROI 排序）、模型兼容性备注。
  - **基础设施改造（最小改动）**:
    - `tools/view/main.go` 增加 `skill-review` 数据源。
    - `tools/view/template.html` 增加 tab/panel/empty-state。
    - `README.md`、`README.zh.md` 同步状态说明与用法。
  - **落地顺序**:
    - Step 1（MVP）: `SKILL.md + 4 个 agent + skill-review.md + dashboard tab`（✅ 已完成，2026-03-25）。
    - Step 2: 增加 `skill-review/bin/lint-skill-review`，把高置信检查迁移到确定性规则（✅ 已完成，2026-03-25）。
    - Step 3: 完成 dashboard 分数徽章与趋势展示（✅ 基础版已完成：分数徽章、历史记录、趋势折线、Critical/High 变化提示）。
- **Effort**: XL
- **Depends on**: code-explore 混合架构升级完成
- **Status**: 🚧 MVP 实施中 (规划见 `docs/skill-review-planning.md`)
...
│       │   ├── SKILL.md
│       │   └── agents/
│       ├── skill-review/        # Skill,审查 Prompt/Skill 工程
│       │   ├── SKILL.md
│       │   └── agents/
