...
- **实施方案（已锁定）**:
  ...
  - **入口策略**:
    - 新增 `skills/judge-the-code/skill-review/SKILL.md`。
    - 自动识别 Skill 项目：目标目录存在 `SKILL.md` 或 `agents/*.md` 时优先走 `skill-review` 路径。
  - **执行流（5 Phase）**:
    - Phase 0: 输入验证（EMPTY/NOT_FOUND/IS_FILE/OK，与现有子 skill 一致）。
    - Phase 1: 结构发现（枚举 `SKILL.md`、`agents/*.md`、`setup`、`bin/*`）。
  ...
  - **模块拆分**:
    - `skill-review/agents/agent1-clarity.md`（Prompt 清晰度）
    - `skill-review/agents/agent2-flow.md`（Phase/流程设计）
    - `skill-review/agents/agent3-safety.md`（注入与权限边界）
    - `skill-review/agents/agent4-orchestration.md`（并行/依赖/容错）
  - **报告规范**:
    - 文件：`.judge-the-code/skill-review.md`
    - 内容：总分（0-100）+ 等级、8 维评分、按严重度分级的 Findings（含路径+行号+修复建议）、Top 5 Fixes（按 ROI 排序）、模型兼容性备注。
  - **基础设施改造（最小改动）**:
    - `skills/judge-the-code/view/main.go` 增加 `skill-review` 数据源。
    - `skills/judge-the-code/view/template.html` 增加 tab/panel/empty-state。
